...
DAYS = 30
# Start date: 30 days ago from "now" — we use a fixed reference for repeatability
REFERENCE_NOW = datetime(2026, 3, 11, 12, 0, 0, tzinfo=IST)
...
class WaterQualityGenerator:
    ...
    UNITS = {"pH": "pH", "DO": "mg/L", "BOD": "mg/L", "COD": "mg/L", "TSS": "mg/L", "Turbidity": "NTU", "Conductivity": "uS/cm", "Temperature": "C", "Nitrates": "mg/L", "Phosphates": "mg/L"}
    ...
    def generate(self, ts: datetime, anomaly_params: set[str] | None = None, anomaly_mult: float = 1.0) -> list[list]:
        hour = ts.hour + ts.minute / 60.0
    ...
    def _sub_wqi(self, param: str, value: float) -> float:
        ...
        if "max" in limits:
            ...
            return min(100.0, 100 * value / limits["min"]) if limits["min"] > 0 else 50.0
        return 50.0
...
class NoiseGenerator:
    ...
    def generate(self, ts: datetime, anomaly_mult: float = 1.0) -> list[list]:
        ...
        metrics_values = {"Leq": leq, "Lmax": lmax, "Lmin": lmin, "L10": l10, "L50": l50, "L90": l90, "Lden": lden}
        rows = []
...
class OCEMSGenerator:
    ...
    BASE_FRACTION = {
        "Thermal Power": 0.55, "Integrated Steel": 0.65, "Sponge Iron": 0.70,
        "Cement": 0.50, "Aluminium Smelting": 0.60, "Coal Mining": 0.45,
        "Coal Washery": 0.40, "Rice Mill": 0.35, "Ferro Alloys": 0.65,
    }
    ...
    def generate(
        ...
        for param in self.factory.emission_params:
            ...
            if a_type == "spike" and apply_anomaly:
                value = base_val * self.rng.uniform(*((anomaly_mult, anomaly_mult + 1) if isinstance(anomaly_mult, float) else anomaly_mult))
                is_anomaly = 1
                ...
                value = max(0, base_val * shift_factor * (1 - anomaly_mult) + self.rng.normal(0, base_val * 0.08))
                is_anomaly = 0
                quality_flag = "valid"
                dahs_status = "online"
                sensor_health = 100.0
            else:
                value = max(0, base_val * shift_factor + self.rng.normal(0, base_val * 0.1))
                is_anomaly = 0
                quality_flag = "valid"
                dahs_status = "online"
                sensor_health = 100.0
...
def get_anomaly_multiplier(ts: datetime) -> dict:
    ...
    for event_day, event in ANOMALY_EVENTS.items():
        if event["type"] == "ocems_spike" and is_in_anomaly_window(ts, event_day, event):
            mult = random.uniform(*event["multiplier"])
            for fid in event["factory_ids"]:
                result["ocems_anomaly"][fid] = {
                    "type": "spike", "multiplier": mult,
        ...
        elif event["type"] == "ocems_stuck" and is_in_anomaly_window(ts, event_day, event):
            for fid in event["factory_ids"]:
                result["ocems_anomaly"][fid] = {
                    "type": "stuck", "multiplier": 1.0,
                    "params": set(event["params"]),
                }

        elif event["type"] == "noise_spike" and is_in_anomaly_window(ts, event_day, event):
            mult = random.uniform(*event["multiplier"])
        ...
        elif event["type"] == "water_deterioration" and is_in_anomaly_window(ts, event_day, event):
            mult = random.uniform(*event["multiplier"])
            for sid in event["station_ids"]:
    ...
    for ts in timestamps:
        ...
        for sid, gen in generators.items():
            if sid in anomalies["water_anomaly"]:
                ...
                rows = gen.generate(ts, anomaly_params=info["params"], anomaly_mult=info["multiplier"])
            else:
...
WATER_COLUMNS = [
    "station_id", "timestamp", "parameter", "value", "unit", "wqi",
    "river_name", "city", "latitude", "longitude",
    "is_anomaly", "anomaly_type", "quality_flag",
]

NOISE_COLUMNS = [
    "station_id", "timestamp", "metric", "value",
    "city", "latitude", "longitude", "zone",
    "day_limit", "night_limit", "is_exceedance",
    "is_anomaly", "anomaly_type", "quality_flag",
]

OCEMS_COLUMNS = [
    "factory_id", "timestamp", "parameter", "value", "unit",
    "emission_limit", "exceedance_pct", "industry_type",
...
def main():
    ...
    parser.add_argument("--clickhouse", action="store_true", help="Insert directly to ClickHouse")
    parser.add_argument("--csv-dir", type=str, default=None, help="Write CSV files to this directory")
    parser.add_argument("--batch-size", type=int, default=CLICKHOUSE_BATCH_SIZE, help="ClickHouse batch size")
    ...
    parser.add_argument("--skip-air", action="store_true", help="Skip air quality generation")
    parser.add_argument("--skip-water", action="store_true", help="Skip water quality generation")
    parser.add_argument("--skip-noise", action="store_true", help="Skip noise generation")
    parser.add_argument("--skip-ocems", action="store_true", help="Skip OCEMS generation")
    ...
    logger.info(f"  Period: {START_DATE.strftime('%Y-%m-%d')} to {REFERENCE_NOW.strftime('%Y-%m-%d')} ({DAYS} days)")
    logger.info(f"  Stations: {len(AIR_STATIONS)} air, {len(WATER_STATIONS)} water, {len(NOISE_STATIONS)} noise")
    ...
    if not args.skip_water:
        ...
        if args.clickhouse:
            insert_to_clickhouse("water_quality_raw", WATER_COLUMNS, water_rows, **ch_kwargs)
        if args.csv_dir:
